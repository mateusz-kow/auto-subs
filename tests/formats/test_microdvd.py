...
from autosubs.models.subtitles import SubtitleSegment, Subtitles, SubtitleWord

# Parser Tests


def test_parse_basic_with_explicit_fps():
...
def test_parse_with_fps_from_header():
...
def test_parse_fps_parameter_overrides_header():
...
def test_parse_no_fps_raises_error():
...
def test_parse_invalid_fps_raises_error(fps_val):
...
def test_parse_timestamp_accuracy(fps, start_frame, end_frame, expected_start, expected_end):
...
def test_parse_large_frame_numbers():
...
def test_parse_skips_invalid_timestamps():
...
def test_generate_basic(sample_subtitles: Subtitles):
...
@pytest.mark.parametrize("fps_val", [0, -25])
def test_generate_invalid_fps_raises_error(sample_subtitles: Subtitles, fps_val: int):
    """Test that a ValueError is raised for zero or negative FPS."""
...
def test_generate_frame_number_accuracy(start, end, fps, expected_start_frame, expected_end_frame):
...
def test_generate_multiline_text():
...
def test_generate_empty_subtitles():
...
def test_round_trip_conversion(sample_subtitles: Subtitles):
    ...
    for original, parsed in zip(sample_subtitles.segments, parsed_subtitles.segments):
