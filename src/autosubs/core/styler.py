...
class StylerEngine:
    ...
    def _convert_override_to_tags(self, override: StyleOverride) -> list[str]:
        tags: list[str] = []
        if override.primary_color:
            tags.append(f"\\c{override.primary_color}")
        if override.font_name:
            tags.append(f"\\fn{override.font_name}")
        ...
        for compiled_rule in self.word_rules:
            ...
            if self._rule_matches(rule_def, word.text, word.start):
                ...
                if rule_def.style_override:
                    tags = self._convert_override_to_tags(rule_def.style_override)
                    for tag in tags:
                        word.styles.append(WordStyleRange(0, len(word.text), tag))
